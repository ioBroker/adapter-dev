...
import { type ExecaChildProcess, execaNode } from '@esm2cjs/execa';
import { gray, green, red } from 'ansi-colors';
import { build, context, type BuildContext, type BuildOptions as ESBuildOptions, type Format } from 'esbuild';
...
async function getReactFilePaths(reactOptions: BuildOptions): Promise<{
    ...
    let entryPoints = await glob(`${reactOptions.rootDir}/${reactOptions.pattern}`);
    entryPoints = entryPoints.filter(
        ep => !ep.endsWith('.d.ts') && !ep.endsWith('.test.ts') && !ep.endsWith('.test.tsx'),
    );
...
async function getTypeScriptFilePaths(typescriptOptions: BuildOptions): Promise<{
    entryPoints: string[];
    tsConfigPath: string;
}> {
    let entryPoints = await glob(`${typescriptOptions.rootDir}/${typescriptOptions.pattern}`);
...
function typeCheckWatch(tsConfigPath: string): ExecaChildProcess {
    console.log();
    console.log(gray(`Type-checking ${tsConfigPath} with tsc in watch mode...`));
    const tscPath = findTsc();
    return execaNode(tscPath, `-p ${tsConfigPath} --noEmit --watch --preserveWatchOutput`.split(' '), {
        stdout: 'inherit',
        stderr: 'inherit',
    });
}
...
async function buildReact(options: BuildOptions): Promise<void> {
    ...
    await build(getReactBuildOptions(false, options, entryPoints, tsConfigPath));
...
async function buildTypeScript(options: BuildOptions): Promise<void> {
    ...
    await build(getTypeScriptBuildOptions(false, options, entryPoints, tsConfigPath));
...
async function buildAll(reactOptions: BuildOptions, typescriptOptions: BuildOptions): Promise<void> {
    await Promise.all([buildReact(reactOptions), buildTypeScript(typescriptOptions)]);
}
...
async function watchReact(options: BuildOptions): Promise<{
    ...
    // 2. type-check with TypeScript (if there are TSX entry points)
    let checkProcess: ExecaChildProcess;
    if (entryPoints.some(e => e.endsWith('.tsx'))) {
...
export async function handleBuildReactCommand(watch: boolean, options: BuildOptions): Promise<void> {
    if (watch) {
...
export async function handleBuildTypeScriptCommand(watch: boolean, options: BuildOptions): Promise<void> {
    if (watch) {
        // In watch mode, we start the ESBuild and TSC processes in parallel and wait until they end
