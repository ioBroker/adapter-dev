...
class TestingTranslator implements Translator {
    translate(text: string, targetLang: string): Promise<string> {
        return Promise.resolve(`Mock translation of '${text}' to '${targetLang}'`);
    }
}
...
class GoogleV3Translator implements Translator {
    ...
    async init(): Promise<void> {
        this.credentials = await readJson(process.env.GOOGLE_APPLICATION_CREDENTIALS || '');
...
class LegacyTranslator implements Translator {
    async translate(text: string, targetLang: string): Promise<string> {
        try {
            ...
            // If a https-proxy is defined as an env variable, use it
