	...
	for (const i18nBase of i18nBases) {
		...
		for (const file of files) {
			...
			try {
				...
				if (dirPath !== baseDirPath && path.basename(dirPath).length === 2) {
					try {
						rmdirSync(dirPath);
						console.log(`Removed empty directory ${path.relative(".", dirPath)}`);
					} catch {
						// Directory not empty or doesn't exist, ignore
					}
				}
			} catch (error) {
				console.log(`Could not delete ${path.relative(".", file)}: file may not exist`);
			}
		}
	}
}
